...
def odt_to_flowables(path: Path) -> list:
    ...
    for para in doc.getElementsByType(odf_text.P) + doc.getElementsByType(odf_text.H):
        txt = text_of(para).strip()
        if not txt:
            continue
        style = (para.getAttribute("stylename") or "")
        is_heading = para.qname[1] == "h" or "head" in style.lower() or "title" in style.lower()
        out.append(Paragraph(G.escape(txt), G.s_h3 if is_heading else G.s_body))
    if not out:
...
def main() -> None:
    argv = sys.argv[1:]
    dist = Path(argv[0]) if argv and not (TESTAKTEN / argv[0]).is_dir() else REPO_ROOT / "dist"
    targets = [a for a in argv if (TESTAKTEN / a).is_dir()]
