# Akte: Bereicherungs-Dreiecksverhaeltnis / Doppelverkauf Oldtimer Bischof-Hellberg / Bonn


<!-- BEGIN gesamt-pdf-section (autogen) -->
## Akte komplett herunterladen

Dieses Aktenpaket gibt es in mehreren Formaten zum Direkt-Download. Das Gesamt-PDF eignet sich zum Lesen, Ausdrucken und für schnelle Durchsichten. Das Akten-ZIP enthält sämtliche Originaldateien (Markdown-Aktenstücke, Tabellen, E-Mails, Fotos, PDFs, DOCX, XLSX) im Originalordnerlayout für eigene Auswertungen. Das Einzel-PDF-ZIP liefert jede einzelne Unterlage als separate, sauber gerenderte PDF im Originalordnerlayout — praktisch, wenn nur einzelne Aktenstücke gebraucht werden.

| Was | Format | Quelle |
| --- | --- | --- |
| Gesamt-PDF (alles in einer Datei, 662 KB) | PDF | [`gesamt-pdf/bereicherung-dreiecksverhaeltnis-doppelverkauf-oldtimer-bischof-bonn_gesamt.pdf`](gesamt-pdf/bereicherung-dreiecksverhaeltnis-doppelverkauf-oldtimer-bischof-bonn_gesamt.pdf) |
| Akten-ZIP (alle Einzeldateien) | ZIP | [testakte-bereicherung-dreiecksverhaeltnis-doppelverkauf-oldtimer-bischof-bonn.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-bereicherung-dreiecksverhaeltnis-doppelverkauf-oldtimer-bischof-bonn.zip) |
| Einzel-PDF-ZIP (jede Unterlage als eigene PDF) | ZIP | [testakte-bereicherung-dreiecksverhaeltnis-doppelverkauf-oldtimer-bischof-bonn-einzelpdfs.zip](https://github.com/Klotzkette/claude-fuer-deutsches-recht/releases/latest/download/testakte-bereicherung-dreiecksverhaeltnis-doppelverkauf-oldtimer-bischof-bonn-einzelpdfs.zip) |

Die ZIP-URLs sind stabil und zeigen immer auf die aktuelle Version. Im Akten-ZIP ist das Gesamt-PDF mit enthalten.

<!-- END gesamt-pdf-section (autogen) -->

**Plugin:** `bereicherungs-und-anfechtungsrecht-pruefer`
**Aktenordner:** `bereicherung-dreiecksverhaeltnis-doppelverkauf-oldtimer-bischof-bonn`
**Erstellt:** März 2026
**Umfang:** 22 Aktenstuecke, 3 Schriftsaetze, 2 Tabellen, 4 E-Mails, 2 PDFs, 3 Grafiken

---

## Sachverhalt (Kurzfassung)

Dr. Reinhart Bischof-Hellberg (68), pensionierter Anästhesist am Universitätsklinikum Bonn
und Sammler historischer Sportwagen, veraeussert seinen Mercedes-Benz 300 SL Gullwing
(Bj. 1955, Schätzwert 1.800.000 EUR) an zwei Käufer: Theresia Bagheri-Aichholz (Wien)
und Heinz-Albrecht Trappenfels (Hamburg). Beide leisten Anzahlungen (600.000 EUR bzw. 850.000 EUR).
Lieferung erfolgt nur an Trappenfels; das Fahrzeug verbrennt zwei Tage später.

Zusätzlich besteht ein Dreiecksverhaeltnis mit der Werkstatt Schmiedeknecht-Restaurations-KG
(78.000 EUR angewiesene Zahlung, Anweisung nach Paragraf 119 BGB angefochten),
eine Insolvenz-Anfechtungslage (AG Hamburg 67 IN 88/26, Insolvenzverwalterin
RAin Dr. Sigrid Drosselberg) sowie eine AnfG-Anfechtung der Übertragung
von sechs weiteren Sportwagen auf Tochter Helena Bischof-Renkert.

**Vertretung:** Spethmann, Kaltenbach & Partner Rechtsanwaltsgesellschaft mbB (Bonn)
**Sachbearbeiter:** RA Dr. Tilman Spethmann
**AZ:** LG Bonn 16 O 211/26, AG Hamburg 67 IN 88/26, LG Bonn 4 O 188/26

---

## Verzeichnisstruktur

```
bereicherung-dreiecksverhaeltnis-doppelverkauf-oldtimer-bischof-bonn/
├── README.md                           (diese Datei)
│
├── 01_sachverhaltsueberblick.md        (Gesamtueberblick + Chronologie)
├── 02_doppelverkauf-kaufvertraege.md   (Kaufvertragsanalyse, beide Kaufvertraege)
├── 03_leistungskondiktion-pruefschema.md  (Vollstaendiges Pruefschema Paragraf 812)
├── 04_dreiecksverhaeltnis-schmiedeknecht.md  (Angewiesene Zahlung 78.000 EUR)
├── 05_insolvenzanfechtung-paragraf-130-inso.md  (Kongruente Deckung)
├── 06_insolvenzanfechtung-paragraf-133-inso.md  (Vorsatzanfechtung)
├── 07_insolvenzanfechtung-rechtfolgen.md        (Paragrafen 143, 144 InsO)
├── 08_anfg-anfechtung-sportwagen-uebertragung.md  (Paragrafen 3, 4 AnfG)
├── 09_saldotheorie-zweikondiktionentheorie.md   (Rueckabwicklung)
├── 10_paragraf-818-wertersatz-brandschaden.md   (Brand und Wertersatz)
├── 11_paragraf-817-beiderseitiger-verstoss.md   (Ausschluss Kondiktionspflicht)
├── 12_verjährung-bereicherungsansprüche.md      (Verjährungsfristen alle Klagen)
├── 13_paragraf-133-inso-vorsatz-vertiefte-pruefung.md  (Vertiefung BGH-Massstab)
├── 14_eingriffskondiktion-nutzungsherausgabe.md (Paragraf 812 Abs. 1 S. 1 2. Alt.)
├── 15_unmoeglichkeit-paragraf-275-bgb.md        (Unmöglichkeit und Folgen)
├── 16_paragraf-119-anfechtung-anweisung.md      (Irrtumsanfechtung Anweisung)
├── 17_bgh-grundsaetze-bereicherungsausgleich-dreieck.md  (BGH-Systematik)
├── 18_anfechtungsklage-drosselberg-prozessual.md  (Prozessrecht InsO)
├── 19_anfg-klage-helena-renkert-prozessual.md   (Prozessrecht AnfG)
├── 20_parallele-anspruchskonkurrenz.md          (Gesamtdarstellung Ansprüche)
├── 21_rechtsgutachten-zusammenfassung.md        (Gutachten Spethmann)
├── 22_mandantenkorrespondenz-spethmann.md       (Mandantenbrief)
│
├── docx/
│   ├── bereicherungsklage-bagheri-aichholz-lg-bonn.docx
│   ├── anfechtungsklage-inso-drosselberg.docx
│   └── anfg-klage-helena-renkert.docx
│
├── xlsx/
│   ├── bereicherungs-zuordnungsmatrix-sportwagen.xlsx
│   └── fristen-wertberechnung-saldotheorie.xlsx
│
├── eml/
│   ├── bagheri-aichholz-rueckforderung.eml
│   ├── drosselberg-anfechtung.eml
│   ├── helena-renkert-benachrichtigung.eml
│   └── schmiedeknecht-werkstatt-anfrage.eml
│
├── pdfs/
│   ├── kaufvertrag-mercedes-300sl-redacted.pdf
│   └── insolvenzeroeffnungsbeschluss-ag-hamburg-redacted.pdf
│
└── jpg/
    ├── mercedes-300sl-gullwing.jpg
    ├── dreiecksverhaeltnis-schema.jpg
    └── insolvenzanfechtung-zeitstrahl.jpg
```

---

## Schwerpunkte und Prüfungsthemen

### Bereicherungsrecht (Paragraf 812 ff. BGB)

| Thema | Aktenstücke |
|---|---|
| Leistungskondiktion (Paragraf 812 Abs. 1 S. 1 1. Alt.) | 02, 03, 11 |
| Eingriffskondiktion (Paragraf 812 Abs. 1 S. 1 2. Alt.) | 14 |
| Dreiecksverhaeltnis / Anweisungsfälle | 04, 17 |
| Saldotheorie vs. Zweikondiktionentheorie | 09 |
| Paragraf 817 S. 2 BGB | 11 |
| Paragraf 818 Abs. 2 BGB (Wertersatz) | 10 |
| Paragraf 818 Abs. 3 BGB (Wegfall Bereicherung) | 04, 10 |
| Verjährung (Paragraf 195 BGB) | 12 |

### Insolvenzanfechtung (Paragrafen 129 ff. InsO)

| Thema | Aktenstücke |
|---|---|
| Paragraf 129 InsO (Grundtatbestand) | 05, 06 |
| Paragraf 130 InsO (Kongruente Deckung) | 05 |
| Paragraf 131 InsO (Inkongruente Deckung) | 05 |
| Paragraf 133 InsO (Vorsatzanfechtung) | 06, 13 |
| Paragrafen 143, 144 InsO (Rechtsfolgen) | 07 |
| Paragraf 146 InsO (Verjährung) | 07, 12 |
| Prozessrecht Paragraf 180 InsO | 18 |

### Gläubigerschutzrecht (AnfG)

| Thema | Aktenstücke |
|---|---|
| Paragraf 3 AnfG (Vorsätzliche Benachteiligung) | 08, 19 |
| Paragraf 4 AnfG (Unentgeltliche Leistung) | 08 |
| Paragraf 11 AnfG (Rechtsfolge Duldung) | 08, 19 |
| Paragraf 2 AnfG (Klagehemmnis Vollstreckungstitel) | 19 |
| Paragraf 15 AnfG (Verjährung) | 12 |

### Allgemeines Schuldrecht / Unmöglichkeit

| Thema | Aktenstücke |
|---|---|
| Paragraf 275 BGB (Unmöglichkeit) | 15 |
| Paragraf 119 BGB (Irrtumsanfechtung) | 16 |
| Paragraf 122 BGB (Vertrauensschaden) | 16 |
| Paragraf 326 BGB (Gefahr bei Unmöglichkeit) | 15 |
| Paragraf 446 BGB (Gefahrtragung) | 10 |

---

## Bearbeitungsvermerk

Diese Akte ist fachlich dem Bereich Bereicherungs- und Anfechtungsrecht zugeordnet.
Die Bearbeitung soll insbesondere folgende Leistungen abdecken:

1. **Vollständige Prüfschema-Erstellung** für Leistungskondiktion und Eingriffskondiktion
2. **Automatische Klassifizierung** von Dreiecksfällen (Anweisungslagen)
3. **Anfechtungsanalyse** InsO + AnfG mit Zeitlinien und Fristen
4. **Saldotheorie-Berechnung** bei komplexen Mehrparteien-Konstellationen
5. **BGH-Rechtsprechungs-Matching** (dejure.org, bundesgerichtshof.de, openjur.de)

---

## Quellen (Gesamtliste)

Folgende Rechtsquellen werden in dieser Akte verwendet:

- Paragraf 812 ff. BGB (Bereicherungsrecht): https://dejure.org/gesetze/BGB/812.html
- Paragraf 129 ff. InsO (Insolvenzanfechtung): https://dejure.org/gesetze/InsO/129.html
- Paragrafen 3, 4, 11 AnfG: https://dejure.org/gesetze/AnfG/3.html
- BGH-Entscheidungen: https://bundesgerichtshof.de
- Rechtsprechungsdatenbank: https://openjur.de
